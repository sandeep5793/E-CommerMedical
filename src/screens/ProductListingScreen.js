...
class ProductListingScreen extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            shortByMixed: [
                ...
                { id: 3, title: 'Short by the latest', value: 'date' },
    ...
    applyFilter = () => {
        let { params } = this.props.navigation.state
        let data = {}
        data['category'] = params && params.categotry && params.categotry.term_id ? (params.categotry.term_id) :
            params && params.item && params.item.id ?
                params.item.id :
                null
        data['per_page'] = 40
            ...
            this.state.dataExit ?
                params && params.onsale ?
                    ...
                    <View style={styles.footer}>
                        <TouchableOpacity
                            activeOpacity={0.9}
                            onPress={() => this.setState({ pageno: this.state.pageno + 1, isRefreshing: true }, () => this.applyFilter())}
    ...
    mainComponent = () => {
        ...
        return (
            <View style={{ paddingBottom: 150, alignItems: 'center', marginTop: 10, }}>

                <FlatList
                    bounces={true}
                    extraData={[this.state, this.props.wishlistData]}
                    numColumns={2}
                            ...
                            <View style={{
                            ...
                            }}>
                                {
                                    <View style={{ flexDirection: 'row', marginHorizontal: 30, justifyContent: 'space-between', paddingBottom: 40 }}>
                                        ...
                                        <TouchableOpacity onPress={() => this.setState({ visible: true, pageno: 1 }, () => { this.applyFilter() })}>
                ...
                <SafeAreaView forceInset={{ top: 'never', bottom: 'always' }}
                    ...
                    <View style={{ flex: 1, backgroundColor: 'white' }}>
                        <Container
                            ...
                            rightItem1={this.state.openFilter ? false : require('../assets/images/whiteSearch/ic_search-white.png')}
                            rightItem2={this.state.openFilter ? false : require('../assets/images/whiteFilter/ic_filter-white.png')}
                            rightItem3={this.state.openFilter ? 'clear all' : false}
                            rightItem1Press={() => this.setState({ openSearchTrue: !this.state.openSearchTrue })}
                            rightItem2Press={() => this.setState({ openFilter: true })}
                            rightItem3Press={() => this.closeFilterModal()}
                            leftItem2={this.state.openFilter ? 'Filter by' : params && params.item ? params.item.name : params && params.categotry ? params.categotry.name : ''}
                            leftItem1Press={this.state.openFilter ? () => this.setState({ openFilter: false }) : () => this.props.navigation.goBack()}
                            mainComponent={() => this.state.openFilter ? this.filterComponent() : this.mainComponent()}
...
export default connect(mapStateToProps, mapDispathToProps)(ProductListingScreen)
